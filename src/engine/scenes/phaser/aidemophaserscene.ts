...
import { createCharacterPredictor } from "../../sandbox/characterpredictor";

export class AiDemoPhaserScene extends Phaser.Scene {
  ...
  async create() { 
    ...
    const worker = this.scene3D.createWebWorker("generateText");

    // load a text file to train the model on
    ...
    const output = await createCharacterPredictor({ 
      message: text,
      seqLength: 10,
      epochs: 25,
      batchSize: 32,
